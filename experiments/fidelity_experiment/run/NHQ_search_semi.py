...
dataset_name_list = ["arxiv", "LAION1M", "tripclick", "yfcc"] * 2

# dataset_list = ["sift1m_UNG_modi"]

NHQ_trade_off = {}
for dataset,  name in zip(dataset_list, dataset_name_list):
    print("processing:", dataset)
    # original_data_path = f"/home/ec2-user/hybrid_hardness/Benchmark/{dataset_name}_original"
    
    data_path = f"/home/ec2-user/hybrid_hardness/fidelity_experiment/{dataset}"
    hardness_path = data_path
    mid_path = os.path.join(data_path, "mid_format")
    NHQ_path = os.path.join(data_path, "nhq_format")

    payloads_path = f"{data_path}/payloads.jsonl"
    tests_file = f"{data_path}/tests.jsonl"
    mapping_path = os.path.join(data_path, "mapping.json")  

    
    base_vector = f"/home/ec2-user/hybrid_hardness/semi-real/filterbenchmark/{name}/{name}_base.fvecs"
    query_fvecs = os.path.join(data_path, f"mid_format/query_vectors_used.fvecs")
...
import pickle
with open(os.path.join(".", "NHQ_trade_off_result_generated.pkl"), "wb") as f:
