...
for dataset_name in dataset_name_list:
    ...
    # for sort_hardness in ["min", "max"]:
        ...
        hardness_path = os.path.join(dataset_path, "hardness_format")
        mid_format = os.path.join(dataset_path,"mid_format")
        query_fvecs_path = os.path.join(dataset_path, f"{dataset_name}_query_equal.fvecs")
        query_label_path = os.path.join(dataset_path, "mid_format/query_label.txt")
        ...
        print("queries.shape =", len(queries)," ", len(queries[0]))
        ...
        # gt = load_groundtruth_bin(os.path.join(dataset_path,"gt.bin"), num_query)
        gt = []
        for test in tests:
            gt.append(test["closest_ids"])
        print("gt.shape =", len(gt)," ", len(gt[0]))
        ...
        pre_filter_path = os.path.join(dataset_path, "pre_filter_format")
