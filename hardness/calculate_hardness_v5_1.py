...
class HybridHardnessEstimator:
    ...
    def compute_H_scan(self):
        k = min(len(self.filtered_ids), 10)
        if k == 0:
            self.alpha = 0
            return 0


        q = self.query_vector.reshape(1, -1)  # (1, d)
        F = self.filtered_vectors             # (m, d)
        B = self.base_vectors                 # (N, d)


        diff_f = F - q
        dists_f = np.sqrt(np.sum(diff_f * diff_f, axis=1))

        dist_Vf = np.partition(dists_f, k-1)[k-1]

        diff_b = B - q
        dists_b = np.sqrt(np.sum(diff_b * diff_b, axis=1))

        dist_V0 = np.partition(dists_b, k-1)[k-1]

        if dist_V0 == 0:
            dist_V0 = dist_Vf

        self.density = dist_Vf / (dist_V0 + 1e-9)
...
def main(data_dir, save_dir):
    ...
    for i, test in enumerate(tqdm(tests)):
        ...
        # 1000개마다 중간 저장
        if (i + 1) % 5000 == 0 or i == 0 or i == 9 or i == 19 or i == 199 or i == 999:
            fname = f'hardness_v5.1_{i+1}.json'
