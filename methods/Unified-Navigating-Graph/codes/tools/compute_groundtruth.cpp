...
int main(int argc, char** argv) {
    std::string data_type, dist_fn, base_bin_file, query_bin_file, base_label_file, query_label_file, gt_file, scenario, gt_counts_file;
    ANNS::IdxType K;
    ...
    try {
        ...
        desc.add_options()("gt_counts_file", po::value<std::string>(&gt_counts_file)->default_value(""),
                           "Filename for writing the number of ground truth per query (txt format)");
    ...
    // write ground truth counts per query to file if specified
    if (!gt_counts_file.empty()) {
        std::ofstream ofs(gt_counts_file);
        if (!ofs) {
            std::cerr << "Failed to open gt_counts_file: " << gt_counts_file << std::endl;
            return -1;
        }
        for (size_t i = 0; i < query_storage->get_num_points(); ++i) {
            size_t count = 0;
            for (size_t j = 0; j < K; ++j) {
                if (groundtruth[i * K + j].first != -1) ++count;
            }
            ofs << count << '\n';
        }
        ofs.close();
