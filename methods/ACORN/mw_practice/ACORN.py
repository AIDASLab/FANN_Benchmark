...
# 먼저 환경변수로 전역 런타임 제한
os.environ["OMP_NUM_THREADS"] = "128"        # OpenMP (FAISS 포함)
...
for num_attribute, card, base_distribution, corr, missing in zip (
  [12,12,12,12,3,12,3,3,3,3,3,3],
  [[1]* 12,[3]* 12,[6]* 12,[12]* 12,   [12]* 3,[3]* 12,   [12]* 3,[12]* 3,[12]* 3,  [12]* 3,[12]* 3,[12]* 3],
  ["zipf","zipf","zipf","zipf","random","random","zipf","zipf","zipf","zipf","zipf","zipf"],
  [[0.5]* 12,[0.5]* 12,[0.5]* 12,[0.5]* 12,   [0.5]* 3,[0.5]* 12,   [0.0]* 3,[1.0]* 3,[0.0,0.5,1.0],  [0.5]* 3,[0.5]* 3,[0.5]* 3],
  [[0.5]* 12,[0.5]* 12,[0.5]* 12,[0.5]* 12,   [0.5]* 3,[0.5]* 12,   [0.5]* 3,[0.5]* 3,[0.5]* 3,  [0.0]* 3,[0.8]* 3,[0.0,0.5,0.8]],
):
...
#                 continue
    ...
    for hardness_type in ["_post", "_corr", "_selec"]:
        ...
        if in_hard_json:
            # JSON 파일 읽기
            if hardness_type in ["_selec", "_corr"]:
                with open(f"{data_path}/hardness/hardness_baseline_10000.json", "r") as f:
                    data = json.load(f)  # list of dicts
            else:
                with open(f"{data_path}/hardness/hardness_v4.1_10000.json", "r") as f:
                    data = json.load(f)  # list of dicts

            # Hardness 값 추출
            if hardness_type == "_post":
                sort_hardness = "Post_Hardness"
            elif hardness_type == "_pre":
                sort_hardness = "Pre_Hardness"
            elif hardness_type == "":
                sort_hardness = "Hardness"
            elif hardness_type == "_corr":
                sort_hardness = "correlation"
            elif hardness_type == "_selec":
                sort_hardness = "selectivity"
            elif hardness_type == "_comb":
                sort_hardness = "select_corr_combine"
            elif hardness_type == "":
                sort_hardness = ""
            else:
                raise ValueError("없는 hardness_type")


            hardness_values = np.array([d[sort_hardness] for d in data])
            print(sort_hardness)

            # 작은 것부터 정렬한 index
            sorted_idx = np.argsort(hardness_values)

            print(sorted_idx)       # Hardness 오름차순 인덱스 (0~9999)
            print(hardness_values[sorted_idx])  # 정렬된 Hardness 값

        else:
            # JSON 파일 읽기
            with open(f"{data_path}/hardness/hardness_v3.3_10000.json", "r") as f:
                data = json.load(f)  # list of dicts

            # Post_Hardness와 Pre_Hardness 동시에 추출
            post_vals = np.array([d["Post_Hardness"] for d in data])
            pre_vals  = np.array([d["Pre_Hardness"] for d in data])

            if hardness_type == "_mul":
                sort_hardness = "mul"
                combined = post_vals * pre_vals
            elif hardness_type == "_harm":
                sort_hardness = "harmonic"
                combined = 2 * post_vals * pre_vals / (post_vals + pre_vals)
            elif hardness_type == "_sum":
                sort_hardness = "sum"
                combined = post_vals + pre_vals
            elif hardness_type == "_geo":
                sort_hardness = "geometric"
                combined = (post_vals * pre_vals) ** 0.5
            elif hardness_type == "_min":
                sort_hardness = "min"
                pre_norm = normalize(pre_vals)
                post_norm = normalize(post_vals)
                combined = np.minimum(pre_norm, post_norm)
            elif hardness_type == "_max":
                sort_hardness = "max"
                pre_norm = normalize(pre_vals)
                post_norm = normalize(post_vals)
                combined = np.maximum(pre_norm, post_norm)
            elif hardness_type == "_min_nn":
                sort_hardness = "nn_min"
                combined = np.minimum(pre_vals, post_vals)
            elif hardness_type == "_max_nn":
                sort_hardness = "nn_max"
                combined = np.minimum(pre_vals, post_vals)
            else:
                raise ValueError("없는 hardness_type")
            ...
            print(sort_hardness)
            print(sorted_idx)              # 인덱스
            print(combined[sorted_idx])    # 정렬된 곱값

        # 예시
        input_file = f"{data_path}/mid_format/query_vectors_ACORN.fvec"
        ...
        # 순서 재배치
        sorted_vectors = vectors[sorted_idx]

        # 다시 저장
        for i in range(10):
            output_file = f"{data_path}/ACORN_format/batch{i}{hardness_type}/query_vectors_ACORN.fvec"
            write_fvecs(output_file, sorted_vectors[i*int(nq/10) : int(nq/10) + i*int(nq/10)])
            print(f"재정렬된 fvec 파일 저장 완료: {output_file}")

        # 예시
        input_file = f"{data_path}/mid_format/query_filter_ACORN.txt"

        # fvec 읽기
        vectors = read_txt(input_file)

        # sorted_idx: 1D 정수 배열/리스트
        idx_list = sorted_idx.tolist() if isinstance(sorted_idx, np.ndarray) else list(sorted_idx)
        sorted_vectors = [vectors[i] for i in idx_list]  # vectors는 list-of-list 유지

        # 다시 저장
        for i in range(10):
            output_file = f"{data_path}/ACORN_format/batch{i}{hardness_type}/query_filters_ACORN.txt"
            write_txt(output_file, sorted_vectors[i*int(nq/10) : int(nq/10) + i*int(nq/10)])
            print(f"재정렬된 fvec 파일 저장 완료: {output_file}")


        # 예시
        input_file = f"/{data_path}/mid_format/query_gt_ACORN.txt"
        ...
        # 다시 저장
        for i in range(10):
            output_file = f"/{data_path}/ACORN_format/batch{i}{hardness_type}/query_gt_ACORN.txt"
            write_txt(output_file, sorted_vectors[i*int(nq/10) : int(nq/10) + i*int(nq/10)])
            print(f"재정렬된 fvec 파일 저장 완료: {output_file}")
