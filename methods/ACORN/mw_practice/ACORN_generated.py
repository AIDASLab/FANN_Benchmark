...
for num, level in zip (
  [1,1,1],
  ["high","low","random"]
):

    data_path = f"/home/mintaek/hybrid_index/Benchmark/generated_{num}_{level}"


    # -----------------------------
    # 1. 스키마(필터 조건) 로드
    # -----------------------------
    with open(f"{data_path}/filters.json", "r", encoding="utf-8") as f:
        schema = json.load(f)
    ...
    input_path = f"{data_path}/payloads.jsonl"  # 한 줄당 하나의 JSON 객체
    ...
    # 원본 .npy 파일 경로
    npy_path = f"{data_path}/base_vectors.npy"
    # 변환된 .fvec 파일 저장 경로
    ...
    input_path = f"{data_path}/tests.jsonl"  # 한 줄당 하나의 JSON 객체
    ...
    print(f"build_and_run_acorn_{num}_{level}")
    ...
    print(f"build_bit_map_{num}_{level}")
    ...
    print(f"build_and_run_acorn_M_gamma_M_beta_{num}_{level}")
    ...
    print(f"run_acorn_search_{num}_{level}")
    ...
    print(f"finish_{num}_{level}")
    num_file = num_file+1
    

gener_list = ["gen_1_high","gen_1_low","gen_1_random"]

ACORN_trade_off = {}

for i, genlis in enumerate(gener_list):
    a = {}
    for j, par in enumerate(params_list):
        par_key = str(par)  # "(16, 64, 8)" 이런 식으로 문자열로 저장
        qps = float(batches[i][j][1])
        avg_recall = float(batches[i][j][0])
        a[par_key] = {"qps": qps, "avg_recall": avg_recall}

    ACORN_trade_off[genlis] = a

save_path = "/home/mintaek/hybrid_index/methods/ACORN/mw_practice/acorn_trade_off_1.json"
with open(save_path, "w") as f:
    json.dump(ACORN_trade_off, f, indent=4)
